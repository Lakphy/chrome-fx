 ...
 * Chrome serializes only this function's source — keep every helper inside it.
 */
export function pageDispatch(op: PageOp, args: unknown[]): unknown {
  try {
    ...
    const missing = (selector: string) => ({ ok: false, error: `not found: ${selector}` });
    ...
    if (op === 'snapshot') {
      ...
      return {
        text: clip(document.body ? visibleText(document.body) : '', 80_000),
    ...
    if (op === 'wait') {
      ...
      return new Promise((resolve) => {
        ...
        const finish = (result: { ok: boolean; error?: string; selector?: string }) => {
    ...
    if (op === 'text') return { ok: true, value: visibleText(el), selector: describe(el) };
...
export function assertPageDispatchInjectable(): void {
  const source = Function.prototype.toString.call(pageDispatch);
  if (!source.includes('querySelector') || !source.includes('snapshot') || !source.includes('outerHTML')) {
    throw new Error('pageDispatch was split by the bundler; executeScript would inject a broken host function');
  }
}
...
export function isSnapshotResult(value: unknown): value is SnapshotResult {
  return Boolean(
    value
    && typeof value === 'object'
    && typeof (value as SnapshotResult).text === 'string'
    && typeof (value as SnapshotResult).html === 'string'
    && typeof (value as SnapshotResult).a11y === 'string',
  );
}
